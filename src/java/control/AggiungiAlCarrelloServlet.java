    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (delIdxParam != null && !delIdxParam.isEmpty()) {
                ...
                if (delIdx >= 0 && delIdx < carrello.size()) carrello.remove(delIdx);
            }
            ...
            if (!giaPresente && idPiatto > 0) {
                ProdottoDAO dao = new ProdottoDAO();
                Prodotto p = dao.getProdottoById(idPiatto);
                if (p != null) carrello.add(new ElementoCarrello(p, quantita, opzioni, prezzoExtra));
            }
            
            session.setAttribute("carrello", carrello);
            out.print("{\"successo\": true}");
        } catch (Exception e) {
            out.print("{\"successo\": false, \"messaggio\": \"" + e.getMessage() + "\"}");
        }
    }
}